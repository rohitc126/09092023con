...
namespace BusinessLayer.DAL
{
    public class DAL_CONCRETE_SPECIMEN
    {
        ...
        public List<CONCRETE_SPECIFICATION_DATALIST> Select_Concrete_Specimen_List(Concrete_List COLLIST)
        {
            ...
            if (ds != null && ds.Tables[0].Rows.Count > 0)
            {
                foreach (DataRow Rows in ds.Tables[0].Rows)
                {
                    ...
                    {
                        Test_ID = Convert.ToInt32(Rows["Test_ID"] == DBNull.Value ? 0 : Rows["Test_ID"]),
                        ...
                        //TestingCount = Convert.ToDecimal(Rows["SAMPLE_COUNT"]),
        ...
        public CONCRETE_SPECIFICATION_EDIT Edit_Concrete_Specimens(int Test_ID)
        {
            ...
            DataSet ds = new DataAccess(sqlConnection.GetConnectionString_SGX()).GetDataSet("[AGG].[USP_INSERT_CONCRETE_SPECIMENS_EDIT]", CommandType.StoredProcedure, param);
            ...
            if (ds != null && ds.Tables[0].Rows.Count > 0)
            {
                    ...
                    _objSPGR.Test_ID = Convert.ToInt32(dt.Rows[0]["Test_ID"]);
                    _objSPGR.Test_No = Convert.ToString(dt.Rows[0]["Test_No"]);
                    //_objSPGR.GRADE_ID = Convert.ToString(dt.Rows[0]["GRADE_ID"] == DBNull.Value ? "" : dt.Rows[0]["GRADE_ID"]);
                    //_objSPGR.GRADE_NAME = Convert.ToString(dt.Rows[0]["GRADE_NAME"] == DBNull.Value ? "" : dt.Rows[0]["GRADE_NAME"]);
                    _objSPGR.Dry_Bulb = Convert.ToString(dt.Rows[0]["Dry_Bulb"]);
                    _objSPGR.Wet_Bulb = Convert.ToString(dt.Rows[0]["Wet_Bulb"]);
                    _objSPGR.Date_Test = Convert.ToString(dt.Rows[0]["Date_Test"]);
            ...
            using (var connection = new SqlConnection(sqlConnection.GetConnectionString_SGX()))
            {
                ...
                try
                {
                    ...
                    { 
                     ...
                     ,new SqlParameter("@Added_By", Emp_Code) 
                     //,new SqlParameter("@FILE_PATH", _CONCRETE.FILE_PATH) 
                    ...
                    if (errorMsg == "")
                    {
                        ...
                        if (_CONCRETE.Concrete_Specimen_Dtl_List != null)
                        {
                            ...
                            foreach (Concrete_Specimen_Dtl item in _CONCRETE.Concrete_Specimen_Dtl_List)
                            {
                               ...
                               ,new SqlParameter("@Test_ID" , (_CONCRETE.Test_ID == null)? 0 : _CONCRETE.Test_ID) 
                               ,new SqlParameter("@Client" , item.Client)
                               ,new SqlParameter("@Id_Mark", (item.Id_Mark == null)? 0 : item.Id_Mark) 
                               ,new SqlParameter("@Dimension_L", (item.Dimension_L == null)? 0 : item.Dimension_L)
                               ,new SqlParameter("@Dimension_B", (item.Dimension_B == null)? 0 : item.Dimension_B)
                               ,new SqlParameter("@Dimension_H", (item.Dimension_H == null)? 0 : item.Dimension_H) 
                               ,new SqlParameter("@Cast_Date",  (item.Date_Of_Casting == null)? (object)DBNull.Value : item.Date_Of_Casting)
                               ,new SqlParameter("@Test_Age",  (item.Age_Test == null)? 0 : item.Age_Test) 
                               ,new SqlParameter("@GRADE_ID",    (item.GRADE_ID == null)? 0 : item.GRADE_ID) 
                               ,new SqlParameter("@Weight", (item.Weight == null)? 0 : item.Weight) 
                               ,new SqlParameter("@Crushing_Load", (item.Crushing_Load == null)? 0 : item.Crushing_Load) 
                               ,new SqlParameter("@Compressive_strength", (item.Compressive_Strength == null)? 0: item.Compressive_Strength) 
                               ,new SqlParameter("@Avg_Compressive", (item.AVG_Compressive_Strength == null)? 0: item.AVG_Compressive_Strength)  
                               ,new SqlParameter("@Remarks",  (item.Remarks == null)? (object)DBNull.Value : item.Remarks) 
                                };
        ...
        public Concrete_Specimen View_Concrete_Specimens(decimal Test_ID)
        {
            ...
            foreach (DataRow row in dt.Rows)
            {
                ...
                dtl.Date_Of_Casting = Convert.ToDateTime(row["Cast_Date"] == DBNull.Value ? 0 : row["Cast_Date"]);
                dtl.Age_Test = Convert.ToInt32(row["Test_Age"] == DBNull.Value ? 0 : row["Test_Age"]);
                dtl.GRADE_ID = Convert.ToInt32(row["Grade_ID"] == DBNull.Value ? "" : row["Grade_ID"]);
                dtl.Weight = Convert.ToDecimal(row["Weight"] == DBNull.Value ? 0 : row["Weight"]);
                dtl.Crushing_Load = Convert.ToDecimal(row["Crushing_Load"] == DBNull.Value ? 0 : row["Crushing_Load"]);
                dtl.Compressive_Strength = Convert.ToDecimal(row["Compressive_strength"] == DBNull.Value ? 0 : row["Compressive_strength"]);
                dtl.AVG_Compressive_Strength = Convert.ToDecimal(row["Avg_Compressive"] == DBNull.Value ? 0 : row["Avg_Compressive"]);
